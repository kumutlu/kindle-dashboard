...
class KindleScriptsTests(unittest.TestCase):
    ...
    def test_low_power_mode_disabled_falls_back_to_sleep(self):
        ...
        calls = (self.sandbox / "calls.log").read_text(encoding="utf-8")
        self.assertIn("sleep 1800", calls)
    ...
    def test_low_power_mode_rtc1_missing_falls_back(self):
        ...
        self.assertIn("[low-power] selected interval: 1800s", logs)
    ...
    def test_low_power_mode_successful_rtc_setup_and_suspend(self):
        ...
        # Create a mock power state file
        ...
        self.create_mock_bin("wget", (
            ...
            "  if [ \"$CALLS\" -ge 3 ]; then\n"
            "    GPID=$(cat \"$DASHBOARD_DIR/kindle-refresh.lock\" 2>/dev/null || cat /tmp/kindle-refresh.lock 2>/dev/null)\n"
            "    if [ -n \"$GPID\" ]; then\n"
            "      kill -s TERM $GPID\n"
            "    fi\n"
            "    exit 0\n"
        ...
        lock_file = self.sandbox / "kindle-refresh.lock"
        self.env["LOCK_FILE"] = str(lock_file)
    ...
    def test_stale_pid_recovery_and_duplicate_process_prevention(self):
        ...
        self.create_mock_bin("wget", "#!/bin/sh\nkill -s TERM $PPID")

        code, stdout, stderr = self.run_script(REFRESH_SH, timeout=5.0)

        # Should overwrite lock and start refresh, then clean it up on exit
        self.assertIn("removing stale lock file for PID 12345", stdout.decode("utf-8"))
    ...
    def test_lock_ownership_cleanup(self):
        ...
        code, stdout, stderr = self.run_script(REFRESH_SH, timeout=5.0)
        self.assertEqual(code, 0)
        # Lock file should still exist and still belong to 12345
        self.assertTrue(lock_file.exists())
        self.assertEqual(lock_file.read_text(encoding="utf-8").strip(), "12345")
